...
_Generated from the source by `scripts/architecture-modules.cjs`. 165 modules, each with the
...
| `capture-manifest.cjs`         | capture-manifest.cjs — the allowlist for what a capture sample may carry.                                            |
...
| `corpus-freeze.cjs`            | corpus-freeze.cjs — freeze the corpus at a known version and hash it.                                                |
| `corpus-frosting-report.cjs`   | corpus-frosting-report.cjs — a shareable report of what the corpus frosting looks like.                              |
| `corpus-manifest.cjs`          | corpus-manifest.cjs — the ledger of every sample in a frozen corpus, with its whole frosting.                        |
...
| `emergency-stop.cjs`           | emergency-stop.cjs — the one control that stops every live input.                                                    |
| `errors.cjs`                   | Error helpers.                                                                                                       |
| `fixture-manifest.cjs`         | fixture-manifest.cjs — the recipe card every fixture carries.                                                        |
...
| `game-screen.cjs`              | game-screen.cjs â€” local OCR preparation and screen classification.                                                 |
...
| `input-adapter.cjs`            | input-adapter.cjs — the contract an input adapter obeys, and the fake used in tests.                                 |
...
| `live-input-adapter.cjs`       | live-input-adapter.cjs — the production input adapter.                                                               |
...
| `navigation-guard.cjs`         | navigation-guard.cjs — the precondition and postcondition rules every input goes through.                            |
| `navigation-runner.cjs`        | navigation-runner.cjs — one action through the guard, the adapter, and the postcondition.                            |
...
| `observation-stamp.cjs`        | observation-stamp.cjs — attach the recognizer version to every observation.                                          |
...
| `recognizer-reports.cjs`       | recognizer-reports.cjs — the ledger of recognizer changes: which hash was justified by which report.                 |
| `recognizer-stamp-writer.cjs`  | recognizer-stamp-writer.cjs — a helper the coordinator calls when a rule change is accepted.                         |
| `recognizer-version.cjs`       | recognizer-version.cjs — the recognizer's own version and the reports that justify every rule change.                |
